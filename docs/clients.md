...
- **Timeout:** `PODCAST_DOWNLOAD_TIMEOUT`, default 30 s, for feed fetches and episode downloads.
...
- **A failed episode download is retried once**, straight away, with a different User-Agent (seen live 2026-09-25). A `503` from Solstein then gets a second chance; after that, ABS doesn't retry on its own.
...
- **Stay well under 30 s:** the first request for a new prefix URL fetches the source synchronously with a 20 s limit; processing on request waits at most 20 s before answering `503` ([`episodes.md`](episodes.md)).
