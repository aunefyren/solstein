...
- **Lifecycle:** a tunnel opens on first use and closes after **5 minutes idle**, so a configured but unused exit costs nothing. It counts its **users** — open connections, and the operation the pool handed it to — and is only idle once all are gone, so a long download is never cut. At `max_tunnels`, the least recently used idle tunnel is closed to make room; if all are busy, the request fails with `ErrTunnelLimit` rather than cut a transfer. All tunnels close on shutdown.
