...
- **Timeout:** `PODCAST_DOWNLOAD_TIMEOUT`, **in milliseconds**, default `30000` (30 s), for feed fetches and episode downloads. The unit is easy to get wrong: set to `600` meaning seconds, every download is abandoned after 0.6 s, which shows up as a cancelled request in Solstein's log (seen while testing, 2026-09-26). Solstein's `processing_wait_seconds` (default 20) is kept inside it; raising one without the other means the request fails on ABS's side instead of coming back as a `503` it can retry ([`episodes.md`](episodes.md)).
...
- **A cancelled download is ABS giving up, not an error:** when its timeout passes, the request is simply dropped. Solstein logs that at debug and answers nothing, since no status can reach the client any more; it used to log an error and answer `500`, which buried real failures under every abandoned download.
...
**A raised timeout on both sides makes the first pass work** (2026-09-26, verified): with `PODCAST_DOWNLOAD_TIMEOUT=600000` on ABS and `processing_wait_seconds: 300` in Solstein, the same 15-episode bulk download arrived **15 of 15 on the first pass, every request answered `200`** — held 23 s to 71 s each (median 56 s) while the episode was cleaned, 13 minutes in total, no `503` and no failed attempt. ABS downloads one episode at a time, so each request simply waits its turn; its queue and progress display coped. The first attempt at this test set the timeout to `600` thinking seconds, and every download was abandoned after **0.6 s** — the unit is milliseconds.
