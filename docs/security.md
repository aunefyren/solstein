...
- **`default_exit`:** the exit for everything that names none — feeds without their own exit, and the Proton server-list refresh. Empty means `direct`, or with the direct exit off, the first VPN exit by name (logged).
- **`direct_exit`** (`auto` by default, `on`, `off`): whether the `direct` exit exists. `auto` is off as soon as `config.json` sets up VPN exits, so an operator who adds a VPN gets no traffic from their own address without asking for it; a fresh install without a VPN still works. Whether the exits load doesn't matter: a VPN that fails to come up must never mean traffic going out directly instead. With it off, a feed or region-diff pair naming `direct` is invalid (region diff's home side becomes a VPN exit in the home country, e.g. Proton `NO`), and `default_exit: direct` stops start-up. It replaced `disable_direct` (2026-09-26): every `config.json` had `"disable_direct": false` written in, the old default, so a new default on that field would never have reached them; `false` is migrated to `auto`, `true` to `off`, and the field dropped.
- **Never a silent fallback:** when the default exit is down, requests fail rather than go out directly. A `default_exit` that doesn't exist or failed to load, or the direct exit off with no VPN exit loaded to take its place, **stops start-up** — stricter than "a broken module stays off", because the operator has said where traffic must go, and leaking would be worse than not running.
