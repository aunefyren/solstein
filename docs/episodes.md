...
- **A request that gets no response** — a connection dropped (seen through a VPN tunnel as a bare `EOF`), or no response headers within 30 s — is retried once after 0.5 s within the same attempt, on a new connection: the client's idle connections are closed first, since every request to a host shares one HTTP/2 connection and the retry mustn't go out on the one that just failed.
