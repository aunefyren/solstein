...
// Read reads a frame's side information into si, which it clears first, so
// one SideInfo can serve every frame (Solstein).
func Read(source FullReader, header frameheader.FrameHeader, si *SideInfo) (*SideInfo, error) {
	*si = SideInfo{}
